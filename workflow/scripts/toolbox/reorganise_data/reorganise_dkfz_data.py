...
def create_directories_and_links(excel_path, base_directory, source_directory):
    ...
    sample_mapping_cols = list(sample_mapping.columns)
    assert set([e in sample_mapping_cols for e in ["Cell Name", "Sample Name"]]) == {
        True
    }
    ...
    for _, row in sample_mapping.iterrows():
        ...
        cell_name = row["Cell Name"].replace("_", "").replace("-", "")
        print(cell_name)
        ...
        for fastq_file in ["R1.fastq.gz", "R2.fastq.gz"]:
            ...
            if os.path.exists(src_file_path):
                ...
                dst_file_renamed = f"{cell_name}.{fastq_file.replace('R1.fastq.gz', '1.fastq.gz').replace('R2.fastq.gz', '2.fastq.gz')}"
                # print(dst_file_renamed)
                # exit()
                # dst_file_renamed = (
                #     dst_file_renamed.rsplit("_", 1)[0]
                #     + "."
                #     + dst_file_renamed.rsplit("_", 1)[1]
                # )
                os.rename(dst_file_path, os.path.join(sample_dir, dst_file_renamed))
                logging.info(
                    f"Symlinked and renamed {src_file_path} to {dst_file_renamed}"
                )

    logging.info(
        "Directories, symbolic links, and file renames have been completed successfully."
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        logging.error(
            "Usage: python script.py <path_to_excel> <base_directory> <source_directory>"
        )
        sys.exit(1)
