...
make_dumbbell <- function(segs_llhs_f, groupname = "Inv", run_shiny = F) {
  ...
  ### PLOT ###
  g <- ggplot(segs_llhs, aes(x = (len / 1000))) +
...
make_table_finaledition <- function(call_llhs_f, group_f, sname_f) {
  ...
  dfsoft <- call_llhs_f2[, c(
    "sum_1010", "sum_0000", "sum_0010", "sum_1000",
  ...
  overview_return <- overview[, c(
    "chrom", "start", "end", "len", "sample", "orig_label",
