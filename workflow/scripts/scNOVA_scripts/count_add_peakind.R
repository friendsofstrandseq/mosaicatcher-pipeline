count_add_peakind <- function(inputfile){
	...
	filename <- inputfile
	filename_spl <- strsplit(inputfile, '.txt')[[1]][1]
	filename_out <- paste0(filename_spl, "_num.txt")
	write.table(data1_resize, filename_out, row.names = FALSE, col.names = TRUE, sep="\t", quote = FALSE)
}
