...
mosaiClassifierCalcProbs <- function(probs, maximumCN = 4, haplotypeMode = F, alpha = 0.05) {
  ...
  for (st in c("CC", "WW", "WC", "CW")) {
    hapStrandStates <- rbind(
      hapStrandStates,
      data.table(
        class = st, haplotype = hapStatus,
  ...
  message(
    "[MosaiClassifier] Expand table by ",
    length(hapStatus),
  ...
  probs <- probs[, .(
    sample, cell, chrom, start, end, class, nb_p, expected, num_bins,
...
mosaiClassifierPostProcessing <- function(probs,
  ...
  message(
    "[MosaiClassifier] The number of segments with 0 prob for all haplotypes is = ",
    segs_max_hap_nb_probs[max_nb_hap_ll == 0, .N]
  )

  # set a uniform prob on sce segs and the segs_max_hap_nb_probs=0
  probs[
    segs_max_hap_nb_probs$max_nb_hap_ll == 0,
