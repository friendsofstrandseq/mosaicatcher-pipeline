...
list_dir = [e.split('/')[-1].replace('.sort.mdup.bam', '') for e in os.listdir("{folder}/all/".format(folder=folder)) if e.endswith(".sort.mdup.bam")]
print(list_dir)
print(expand("{folder}/lite_final_{chrom}/{cell}.sort.mdup.bam", folder=folder, cell=list_dir, chrom=chrom))

rule all:
    input:
        expand("{folder}/lite_final_{chrom}/{cell}.sort.mdup.bam.bai", folder=folder, cell=list_dir, chrom=chrom)
