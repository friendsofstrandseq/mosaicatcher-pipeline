...
df_segdups_hg19_lite = pd.read_csv(
    "/g/korbel2/weber/workspace/mosaicatcher-update/workflow/data/segdups/segDups_hg19_lite.bed",
    sep="\t",
    names=["chromStart", "chromEnd", "name"],
)
df_segdups_hg19_lite.index.name = "chrom"
df_segdups_hg19_lite = df_segdups_hg19_lite.reset_index()

merge_df = pd.merge(df_segdups_hg38_full, df_segdups_hg19_lite, on="name")
merge_df = merge_df[col_list + ["chrom_hg38", "chromStart_hg38", "chromEnd_hg38"]]

print(merge_df)
merge_df.to_csv(
    "/g/korbel2/weber/workspace/mosaicatcher-update/workflow/data/segdups/segDups_hg19_UCSCtrack.bed.gz",
    index=False,
    sep="\t",
    compression="gzip",
)
