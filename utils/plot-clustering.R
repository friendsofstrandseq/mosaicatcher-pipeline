...
genome_bins<-read.table("/Users/jeong/Documents/Strand_Seq/TALL_analysis/sv_calls/bin_200kb_all.bed", sep = '\t', header=F, comment.char = "") ##This folder need to be changed ( /path/to/genome bins bed file )
list_directory <- dir("./", full.names=TRUE)

for (m in 1:(length(list_directory))){
...
pdf(plotname, width = 11, height = 10 )

for (k in 1:length(files)){

filename<-strsplit(files[k], './/')[[1]][2]
data1<-read.table(files[k], sep = '\t', header=T, comment.char = "")
...
res<-pheatmap(result, border_color = NA, show_rownames=T, show_colnames=F, cluster_cols=F, cluster_rows=T, clustering_method="ward.D", scale="none", col=mycol, cex=0.7, main=filename, annotation_col = col_annotation, annotation_colors = anno_colors, breaks = breaksList, annotation_legend = FALSE)
...
ht1 = Heatmap(mat, name = "", col=colors, heatmap_legend_param = list(labels = sv_list_sub),cluster_rows = FALSE, cluster_columns = FALSE, column_title = filename, row_names_gp = gpar(fontsize = 5), column_names_gp = gpar(fontsize = 1), column_title_gp = gpar(fontsize = 7, fontface = "bold"), top_annotation = ha_column)
...
files <- list.files(paste0(list_directory[m], '/100000_fixed_norm.selected_j0.1_s0.5'), pattern=".txt$", full.names=TRUE)
plotname<-paste0("Mosaic_plot_", strsplit(list_directory[m], './/')[[1]][2], "_chromosome.pdf")
pdf(plotname, width = 11, height = 5 )
...
res<-pheatmap(result, border_color = NA, show_rownames=T, show_colnames=F, cluster_cols=F, cluster_rows=T, clustering_method="ward.D", scale="none", col=mycol, cex=0.5, main=filename)
